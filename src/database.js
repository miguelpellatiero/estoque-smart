...
export async function updateProduct(id, data, photoFiles = []) {
  ...
  if (updateData.name) updateData.nameLower = updateData.name.toLowerCase();
  ...
  if (quantityChangeData) await addDoc(movementsRef, quantityChangeData);

  if (photoFiles && photoFiles.length) {
    const { uploadProductPhoto } = await import("./storage.js");
    for (const photoFile of photoFiles) {
      await uploadProductPhoto(id, photoFile, { address: updateData.address || '', source: 'product' });
    }
  }
}
...
export async function searchProducts(term) {
  ...
  for (const q of queries) {
    ...
    snapshot.forEach(doc => {
      ...
      if (!results.find(p => p.id === item.id) && item.active !== false) results.push(item);
    });
  }
